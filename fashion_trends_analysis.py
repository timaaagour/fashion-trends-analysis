...
fig, axes = plt.subplots(1, 3, figsize=(16, 6), facecolor=CREAM,
                         gridspec_kw={"width_ratios": [2, 1.2, 1.5]})
...
ax1.legend(frameon=False, fontsize=9, loc="upper right", bbox_to_anchor=(1, 1))
...
ax3.axis("off")
insights_data = [
    ("Y2K Fashion",       BLUE, "Stable ~58/100"),
    ("Quiet Luxury",      GOLD, "Pic 2023, recul"),
    ("Dopamine Dressing", PINK, "Explosion 2025"),
]
for i, (label, color, text) in enumerate(insights_data):
    y = 0.75 - i * 0.28
    ax3.add_patch(mpatches.FancyBboxPatch((0.05, y - 0.1), 0.9, 0.22,
                  boxstyle="round,pad=0.02", facecolor="white",
                  edgecolor=color, linewidth=2, transform=ax3.transAxes))
    ax3.text(0.12, y + 0.06, f"■ {label}", ha="left", va="center",
             fontsize=10, fontweight="bold", color=color, transform=ax3.transAxes)
    ax3.text(0.12, y - 0.04, text, ha="left", va="center",
             fontsize=9, color=DARK, transform=ax3.transAxes)
...
for i, (color, label, text) in enumerate(conclusions):
    ...
    ax.text(0.45, y, text, ha="left", va="center",
